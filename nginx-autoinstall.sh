...
1)
	...
	*)
	...
	./configure $NGINX_OPTIONS $NGINX_MODULES --with-cc-opt="-O3 -march=native -flto -funsafe-math-optimizations -fstack-protector-strong --param=ssp-buffer-size=4 -Wformat -Werror=format-security -Wp,-D_FORTIFY_SOURCE=2 -DTCP_FASTOPEN=23"
	make -j "$(nproc)"
	make install
